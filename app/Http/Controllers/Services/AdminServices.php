...
class AdminServices extends BaseController
{
    ...
    public static function forget($request)
    {
        try {
            ...
            $userInfo = Admin::where('email', $input['email'])->first();
            if($userInfo == null) {
    ...
    public static function reset($request)
    {
        try {
            ...
            $userInfo = Admin::where(['email' => $input['email'], 'reset_code' => $input['code']])->first();
            if($userInfo == null){
    ...
    public static function profile_details($request)
    {
        try {
            ...
            $user = Admin::with('media')->where('id', $user_id)->first();
